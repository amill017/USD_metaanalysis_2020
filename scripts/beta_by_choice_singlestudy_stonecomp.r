...
bind <- as.data.frame(cbind(Sample, r2))
r4 <- bind[!grepl("blank", bind$r2),]  ##dm filter
#r5 <- r4[!grepl("blank", r4$x2),]  ##dm filter
##Subset beta
subs <- as.factor(r4$Sample)  ##if choose urine, all are urine
...
#quartz()
pdf(file=sprintf("data/downloads/beta_%s_%s.pdf",type_save,r_save))
plot(pd, label.cex=1.5, ellipse=TRUE, hull=FALSE, seg.lty="dashed", cex=0.5, cex.main=1.5, cex.sub=0.01, cex.lab=1.1, axes=c(1,2))
mtext("p-values", at=0.9)
legend('topright', legend=rp, bty='n')
#quartz.save(file=sprintf("full_data/beta_%s_%s_%s.png",type_save,r_save), type="png")
dev.off()
