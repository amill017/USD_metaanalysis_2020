...
library(vegan)
library(mefa)
beta <- as.dist(read.csv("data/beta/weighted_unifrac_dm.txt", header=TRUE, row.names=1, sep="\t"))
map <- read.csv("data/current_map_sorted.txt", header=TRUE, sep="\t", stringsAsFactors = FALSE)
...
##Subset beta
subs <- r5$Sample  ##if choose urine, all are urine
