...
alpha <- read.csv("data/current_alpha_sorted.txt", header=TRUE, sep="\t")
map <- read.csv("data/current_map_sorted.txt", header=TRUE, sep="\t")
bind <- cbind(alpha, map)
args <- commandArgs(TRUE)
##Select sample type
sample_type <- as.name(args[1])
...
r4 <- droplevels(r2)
x4 <- droplevels(x2)
res1 <- aov(PD_whole_tree ~ r4 + x4 + r4*x4, data=x3)
...
boxplot(PD_whole_tree~r4*x4, ylab="Phylogenetic Diversity", data=x3, main=c(title), col=(c("red", "blue")))  
leg <- paste(r, rp)
